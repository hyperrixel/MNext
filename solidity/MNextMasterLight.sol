...
/*
 * This is a less safe varionat of MNextMaster.
 *
 * Common stock EVMs like the one of the Ethereum ecosystem doesn't have enough
 * stack deepness allocated to perform full security.
 *
 * In this light version some functions doesn't perform some security checks to
 * reduce stack needs.
 *
 * Differences of this light version:
 *  - transactBankToUser():
 *      doesn't check onlyExistingBank(), onlyExistingUser()
 *  - clearCeck()
 *      doesn't perform lockUser(msg.sender)
 *  - transactBankToBank()
 *      doesn't check onlyExistingBank() for both sides
 */
...
contract MNextMaster is MNextDataModel{
    ...
    function transactBankToBank(uint16 id, uint256 keyId,
                                ...
                                uint256 amount) lockBank(id)
                                onlyValIdBankAction(id, keyId, passPhrase)
                                lockBank(target) external returns (bool) {
